...
export function OnboardingGate({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      if (active) setStatus(step === 4 ? "ready" : "onboarding");
  ...
  return <View style={{ flex: 1, backgroundColor: "#F7F9FC", alignItems: "center", justifyContent: "center", gap: 20 }}>
    {status === "error" ? <>
      <Text><Translated text={"We couldn’t load your account. Please try again."} /></Text>
      <Pressable accessibilityRole="button" onPress={() => setAttempt(a => a + 1)}><Text style={{ color: "#0072B5" }}><Translated text={"Try again"} /></Text></Pressable>
      <Pressable accessibilityRole="button" onPress={signOut}><Text><Translated text={"Sign out"} /></Text></Pressable>
