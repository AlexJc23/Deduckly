...
export function ReportSection({ title, description, children }: PropsWithChildren<{ title: string; description?: string }>) {
  useLanguage();
  return <View style={s.section}><Text accessibilityRole="header" style={s.sectionTitle}>{<Translated text={title} />}</Text>
    {description && <Text style={s.description}>{<Translated text={description} />}</Text>}{children}</View>;
}
export function ReportRow({ label, value, detail, color }: { label: string; value: string; detail?: string; color?: string }) {
  ...
  return <View style={[s.row, (!isTablet || fontScale > 1.2) && s.stacked]}>
    <View style={s.rowCopy}><Text style={s.label}>{<Translated text={label} />}</Text>{detail && <Text style={s.detail}><Translated text={detail} /></Text>}</View>
  ...
  return <View onLayout={event => setAvailableWidth(event.nativeEvent.layout.width)}
    ...
    {[{ label: "Recorded income", impact: reportImpact(report.total_income), value: money(report.total_income), detail: "Income entries in this period" },
      ...
      <View key={item.label} style={[s.metric, columns ? s.metricColumn : s.metricFull, item.impact && { borderColor: item.impact.borderColor, backgroundColor: item.impact.backgroundColor }]}>
        ...
        <Text style={s.detail}>{<Translated text={item.detail} />}</Text>
      </View>)}
  </View>;
}
export function ReportDeductions({ report }: { report: CurrentReport }) {
  ...
  return <View style={s.card}>
    ...
    <View style={[s.highlight, { backgroundColor: reportImpact(report.net_profit).backgroundColor }]}><Text style={s.label}><Translated text={"Income after deductions"} /></Text><Text selectable numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[s.heroValue, { color: reportImpact(report.net_profit).color }]}>{money(report.net_profit)}</Text>
      <Text style={s.detail}><Translated text={"Recorded income − total deductions. This is the report’s net profit calculation, not income minus recorded spending."} /></Text></View>
  </View>;
}
export function ReportTaxes({ report }: { report: CurrentReport }) {
  ...
  return <View style={s.card}>
    ...
    <Text style={s.note}><Translated text={"These figures are estimates from your recorded data, not a final tax bill or a payment due. Your final tax outcome can differ."} /></Text>
  </View>;
}
export function ReportLoadState({ loading, retry }: { loading: boolean; retry: () => void }) {
  useLanguage();
  return <View style={s.state}>{loading ? <><ActivityIndicator color="#0072B5" /><Text style={s.description}><Translated text={"Loading this report…"} /></Text></>
    : <><Text accessibilityRole="header" style={s.sectionTitle}><Translated text={"Couldn’t load this report"} /></Text><Text style={s.description}><Translated text={"Try again or choose another period. Your records have not been changed."} /></Text>
      <Pressable accessibilityRole="button" onPress={retry} style={s.retry}><Text style={s.retryText}><Translated text={"Try again"} /></Text></Pressable></>}</View>;
}
